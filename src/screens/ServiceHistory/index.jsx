...
function ServiceHistoryCard({item}) {
  const navigation = useNavigation();

  return (
    <TouchableOpacity
      style={{marginBottom: 20, borderRadius: 15}}
      onPress={() => navigation.navigate('ServiceHistoryDetail', {item})}>
...
export default function ServiceHistory() {
  ...
  const handleGetHistory = () => new ServiceClient().list().then(setHistory);
  useEffect(() => {
  ...
  return (
    <BaseContainer>
      <View style={styles.subContainer}>
        <HeadingWithBack>Service History</HeadingWithBack>
      </View>
          ...
          emptyHelp={
            <View style={{textAlign: 'center'}}>
              <Heading style={{textAlign: 'center'}} level={5}>
                There are no services requests.
              </Heading>
              <View style={{paddingHorizontal: 20, marginTop: 20}}>
                <Button onPress={() => navigation.navigate('RequestService')}>
                  Request Service
                </Button>
              </View>
            </View>
